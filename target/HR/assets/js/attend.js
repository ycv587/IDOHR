...
$(window).resize(function() {
	$("#attend-table").bootstrapTable('resetView', {
		height: tableHeight()
	})
});
...
/* 加载表格内容 */
function initTable() {
	$("#attend-table").bootstrapTable({
		method: 'get', // 请求方式
		contentType: 'application/x-www-form-urlencoded',
		url: '/attend/show', // 请求后台的URL
...
function selectAttend() {
	...
	startDate = dateToMs($("#date-start").val());
	endDate = dateToMs($("#date-end").val());
	batchStr = $("#setno-select option:selected").attr("id");
	$('#attend-table').bootstrapTable("refresh", {
		url: "/attend/show"
	});
	toastr.success('查询成功!');
	startDate = 0;
